...
export const validaCaracteres = (text) => {
  ...
  if (text) {
    return text
      ...
      .match(/[\wà-úÀ-Ú\s\-`~!@#$%^&*()+=[{\]}\\\\'<,.>?/"";:]/g)
      .join("") // retorna string com os caracrters do regex
...
export const getTemContrato = (categoria) => {
  return categoria === "CONT" || categoria === "CONT1" || categoria === "PARC";
};
...
export const getElapsedTime = (initialDate, initialTime) => {
  ...
  const initialMs = new Date(x).getTime();
  const finalMs = new Date().getTime();

  const diffSeconds = Math.floor((finalMs - initialMs) / 1000);
  ...
  if (days === 0 && hours === 0 && minutes === 0) return "agora";
