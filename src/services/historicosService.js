...
export const getUltimosTelefonesByCodEmpresa = async (codEmpresa) => {
  try {
    var resp = await api.get(
      `/api/v1/suporte/historicos/${codEmpresa}/codempresa`,
    );
  } catch (error) {
  ...
  if (resp.data.length === 2) {
    const historico2 = `${resp.data[1].ContatoChamadasHistorico} ${resp.data[1].TelefoneChamadasHistorico}`;
    ...
    if (historico2.toLowerCase() !== historico1.toLowerCase()) {
      if (contato2 === contato1) {
        response = response + ` e ${resp.data[1].TelefoneChamadasHistorico}`;
      } else {
        response =
          response +
...
export const get = async (codigoClente) => {
  try {
    var resp = await api.get(
      `/api/v1/suporte/historicos/${codigoClente}?idEmpresaChamadas=1`,
    );
  } catch (error) {
...
export const getByTelefone = async (idEmpresa, telefone) => {
  try {
    var resp = await api.get(
      `/api/v1/suporte/historicos/getHistoricosByTelefone/${telefone}?idEmpresa=${idEmpresa}`,
    );
  } catch (error) {
    console.error("ErrorMessage (historicosService.getByTelefone): ", error);
    return null;
  }
  return resp.data;
};
