...
const ChamadasForm = (props) => {
  ...
  const [codEmpresaChamadas] = useState(
    store.getState().chamadasState.CodEmpresaChamadas,
  );
  const [empresaChamadas] = useState(
    store.getState().chamadasState.EmpresaChamadas,
  );
  const [contatoChamadas, setContatoChamadas] = useState(
    store.getState().chamadasState.ContatoChamadas,
  );
  const [telefoneChamadas, setTelefoneChamadas] = useState(
    store.getState().chamadasState.TelefoneChamadas,
  );
  const [telefone2Chamadas, setTelefone2Chamadas] =
    useState(store.getState().chamadasState.Telefone2Chamadas) || "";
  const [obs1Chamadas, setObs1Chamadas] = useState(
    store.getState().chamadasState.Obs1Chamadas,
  );
  const [obs2Chamadas, setObs2Chamadas] = useState(
    store.getState().chamadasState.Obs2Chamadas,
  );
  const [obs3Chamadas, setObs3Chamadas] = useState(
    store.getState().chamadasState.Obs3Chamadas,
  );
  const [obs4Chamadas, setObs4Chamadas] = useState(
    store.getState().chamadasState.Obs4Chamadas,
  );
  const [statusChamadas, setStatusChamadas] = useState(
    store.getState().chamadasState.StatusChamadas,
  );
  const [analistaChamadas, setAnalistaChamadas] = useState(
    store.getState().chamadasState.AnalistaChamadas,
  );

  const [incluidoPorChamadas] = useState(
    store.getState().chamadasState.IncluidoPorChamadas,
  );
  const [atendidoPorChamadas] = useState(
    store.getState().chamadasState.AtendidoPorChamadas,
  );
  const [baixadoPorChamadas] = useState(
    store.getState().chamadasState.BaixadoPorChamadas,
  );
  ...
  const [ultimosTelefonesHistorico, setUltimosTelefonesHistorico] =
    useState("");
  ...
  useEffect(() => {
    ...
    if (evento === "atender" && !analistaChamadas)
      setAnalistaChamadas(store.getState().loginState.loggedUser);
  ...
  return (
    <div id="chamadas-form" className="chamadas-form-container">
      ...
      <div className="chamadas-form-buttons">
        <button
          className="chamadas-form-button"
        ...
        <button
          className="chamadas-form-button-sair"
          onClick={() => {
            history.push("/");
          }}
        >
        ...
        >
          ...
          {!!temContrato && (
            <div className="chamadas-form-info-col">
              <div className="chamadas-form-info-contrato">CONTRATO</div>
            </div>
          )}
          ...
          <div className="chamadas-form-info-col">
            ...
            <div
              style={{ fontSize: "0.9rem" }}
            >{`(${codEmpresaChamadas})  ${empresaChamadas}`}</div>
          </div>
      ...
      <div className="chamadas-form-content">
        ...
        {/* Observações */}
        <div
          style={{ display: "block", flexDirection: "row", flexWrap: "wrap" }}
        >
          <div>
            {" "}
            {/* Observações */}
  ...
  async function getCliente(codCliente) {
    if (codCliente) {
      ...
      setRestricaoCliente(
        cliente.ClienteNetClienteRestricao === "T" ? true : false,
      );
      setBloqueadoCliente(
        cliente.ClienteNetClienteBloqueado === "T" ? true : false,
      );
  ...
  function handleSaveButton() {
    ...
    const emAlmoco =
      obs1Chamadas.toLowerCase() === "almoco" ||
      obs1Chamadas.toLowerCase() === "almoço";

    const analista =
      emAlmoco && !analistaChamadas ? contatoChamadas : analistaChamadas;
    ...
    const updateData = {
      ...
      DataAltChamadas: utils.formattedDateYearFirst(
        store.getState().chamadasState.DataAltChamadas,
      ),
      ...
      Obs5Chamadas: utils.validaCaracteres(
        store.getState().chamadasState.Obs5Chamadas,
      ),
      ...
      StatusChamadas: !emAlmoco
        ? utils.validaCaracteres(statusChamadas)
        : "Baixado",
