              ...
              >
                ...
                >
                  ...
                  {chamada.SituacaoChamadas === "Atendendo" && (
                    <div>
                      {utils.getElapsedTime(
                        chamada.DataAltChamadas,
                        chamada.HoraAltChamadas,
                      )}
                    </div>
                  )}
                </td>
  ...
  if (arr) {
    for (let item of arr) {
      if (item && item.BaixadoPorChamadas) {
        let analista = item.BaixadoPorChamadas.substr(7, 20)
          .split(" ", 1)
          .toString();
        ...
        let qt = arr.filter(
          (it2) =>
            analista ===
            it2.BaixadoPorChamadas.substr(7, 20).split(" ", 1).toString(),
        ).length;
...
function setVariables(chamada) {
  ...
  const pendente =
    chamada.SituacaoChamadas === "Pendente" ||
    chamada.SituacaoChamadas === "Pend.Urgen";
